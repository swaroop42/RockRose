...
/*
 * Adjust the query arguments to accomodate archives
 */
function listable_facetwp_archive_query( $query_args ) {
	$uri = FWP()->facet->http_params['uri'];

	if ( false !== strpos( $uri, 'listing-category') ) {
		$query_args['tax_query'][] = array(
			'taxonomy'	=> 'job_listing_category',
			'field'		=> 'slug',
			'terms'		=> basename( $uri ),
		);
	} elseif ( false !== strpos( $uri, 'listing-type') ) {
		$query_args['tax_query'][] = array(
			'taxonomy'	=> 'job_listing_type',
			'field'		=> 'slug',
			'terms'		=> basename( $uri ),
		);
	} elseif ( false !== strpos( $uri, 'listing-region') ) {
		$query_args['tax_query'][] = array(
			'taxonomy'	=> 'job_listing_region',
			'field'		=> 'slug',
			'terms'		=> basename( $uri ),
		);
	} elseif ( false !== strpos( $uri, 'listing-tag') ) {
		$query_args['tax_query'][] = array(
...
function listable_facetwp_query_args( $query_args, $facet ) {
	...
	$query_args = listable_facetwp_archive_query( $query_args );
